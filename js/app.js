(() => {
  ...
  function bindEvents() {
    ...
    addEventListener("thundershadow-conflict", (event) => showConflict(event.detail));
    $("syncStatusBtn").addEventListener("click", () => {
      const cloud = window.ThunderShadowCloud;
      if (!cloud?.syncNow) {
        showToast("Cloud sync is still initializing.");
        return;
      }
      Promise.resolve(cloud.syncNow({ background: false, reason: "topbar-sync-now" }))
        .catch((error) => showToast(error?.message || "Cloud sync failed."));
    });
    $("retrySyncBtn").addEventListener("click", () => { if (!window.ThunderShadowCloud?.isAuthorized?.()) window.ThunderShadowCloud?.connect?.().catch((error) => showToast(error.message)); else window.ThunderShadowSync.replay(); });
    addEventListener("thundershadow-server-event", (event) => handleServerEvent(event.detail).catch((error) => showToast(error.message))); addEventListener("thundershadow-sync-reconnected", () => reconcileCurrentForm()); addEventListener("thundershadow-storage-recovered", () => { showToast("Browser IndexedDB was unresponsive. Trying recovery storage…"); }); addEventListener("thundershadow-storage-emergency", () => { showToast("IndexedDB is unavailable in this browser profile. ThunderShadow switched to emergency local storage so the app can keep working."); }); addEventListener("thundershadow-cloud-merged", async (event) => {
      // A local-only upload does not change the records already rendered in this
      // tab. Skip the expensive full-library read + logger/library rerender unless
      // Drive actually supplied remote state.
      if (event?.detail?.remoteApplied === false) return;
      try { const forms = normalizeForms(await apiRequest("/api/forms")); state.forms = forms; if (state.currentForm) { const merged = forms.find((form) => form.id === state.currentForm.id); if (merged) { state.currentForm = merged; state.currentEntryNumber = merged.entries.some((entry) => entry.entryNumber === state.currentEntryNumber) ? state.currentEntryNumber : (merged.currentEntry || merged.entries.at(-1)?.entryNumber || 1); renderLogger(); } else showLibrary(); } else renderLibrary(); } catch (error) { showToast(error.message); }
    });
